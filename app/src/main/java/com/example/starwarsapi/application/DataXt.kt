...
import com.example.starwarsapi.presentation.BaseViewModel
...
fun <T> MutableLiveData<ViewState<T, ViewModelStatusEnum>>.postSuccess(data: T) {
    this.postValue(
        ViewState(data, ViewModelStatusEnum.SUCCESS)
    )
}

fun <T> MutableLiveData<ViewState<T, ViewModelStatusEnum>>.postError(throwable: Throwable) {
    this.postValue(
        ViewState(error = throwable, status = ViewModelStatusEnum.ERROR)
    )
}
